...
export class LendUnrealdnMarketvalueService {
  ...
  getKeepRate(params: SearchParams): Observable<UnrealSum[]> {
    let endpoint: string = `${params.APISERVER}${this.baseApiUrl}`; // 定義 API 端點

    // 如果使用模擬數據，則更改端點為本機 JSON 文件
    if (environment.apiMock) {
      endpoint = `assets/mock/api/v1/file/lend-unrealdn-marketvalue.xml`;
    }
    const httpParams = new HttpParams({
      fromObject: {
        ...params,
        Qtype: QtypeParamsSetting.LEND_UNREAL_DN_MARKET_VALUE,
      },
    });
