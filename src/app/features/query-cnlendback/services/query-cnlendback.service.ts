...
export class QueryCnlendbackService {
  ...
  getKeepRate(params: SearchParams): Observable<CnLendBack[]> {
    let endpoint: string = `${params.APISERVER}${this.baseApiUrl}`; // 定義 API 端點

    // 如果使用模擬數據，則更改端點為本機 JSON 文件
    if (environment.apiMock) {
      endpoint = `assets/mock/api/v1/file/query-cnlendback.xml`;
    }
    const httpParams = new HttpParams({
      fromObject: { ...params, Qtype: QtypeParamsSetting.QUERY_CNLENDBAC },
    });
